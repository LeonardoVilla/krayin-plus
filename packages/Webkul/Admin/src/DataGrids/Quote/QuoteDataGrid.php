...
class QuoteDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder(): Builder
    {
        ...
        $this->addFilter('document_type', 'quotes.document_type');
    ...
    public function prepareColumns(): void
    {
        ...
        $this->addColumn([
            'index' => 'document_type',
            'label' => trans('admin::app.quotes.index.datagrid.document-type'),
            'type' => 'string',
            'sortable' => true,
            'filterable' => true,
            'filterable_type' => 'dropdown',
            'filterable_options' => [
                ['id' => 1, 'name' => trans('admin::app.quotes.index.datagrid.quote'), 'value' => 'quote'],
                ['id' => 2, 'name' => trans('admin::app.quotes.index.datagrid.invoice'), 'value' => 'invoice'],
            ],
            'closure' => function ($row) {
                if ($row->document_type === 'invoice') {
                    return '<span class="rounded-md bg-green-100 px-2 py-1 text-xs font-medium text-green-700">'.trans('admin::app.quotes.index.datagrid.invoice').'</span>';
                }

                return '<span class="rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-700">'.trans('admin::app.quotes.index.datagrid.quote').'</span>';
            },
        ]);
    ...
    public function prepareActions(): void
    {
        ...
        if (bouncer()->hasPermission('quotes.edit')) {
            $this->addAction([
                'index' => 'convert_to_invoice',
                'icon' => 'icon-right-arrow',
                'title' => trans('admin::app.quotes.index.datagrid.convert-to-invoice'),
                'method' => 'POST',
                'url' => fn ($row) => route('admin.quotes.convert_to_invoice', $row->id),
            ]);
        }
